...
class RayTracer {
    ...
    private getNaturalColor(thing: Thing, pos: Vector, norm: Vector, rd: Vector, scene: Scene) {
        ...
        return scene.lights.reduce((e, l) => addLight(e, l), ColorTS.defaultColor);
    }
    ...
    render(scene: Scene, ctx: VideoChip, screenWidth: number, screenHeight: number) {
        ...
        for (let y = 0; y < screenHeight; y++) {
            for (let x = 0; x < screenWidth; x++) {
                ...
                const [r, g, b] = [c.r as IntRange<0, 255>, c.g as IntRange<0, 255>, c.b as IntRange<0, 255>]
                ctx.FillRect(vec2(x, y), vec2(x, y), Color(r, g, b));
            }
        }
    }
}
...
type Tracer = Gadget & WithVideoChip & WithLcd & WithCPU;
declare const gdt: Tracer;
